...
export default function ContentManagementPage() {
  ...
  const [sortKey, setSortKey] = useState<'views' | 'likes' | 'comments' | 'date' | null>(null);
  ...
  const [activePopoverVideoId, setActivePopoverVideoId] = useState<number | null>(null);
  ...
  const handleDeleteVideo = async (videoId: number) => {
    setActivePopoverVideoId(null);
    if (window.confirm('Bạn có chắc chắn muốn xóa video này không? Thao tác này không thể hoàn tác.')) {
      try {
  ...
  const handleSortToggle = (key: 'views' | 'likes' | 'comments') => {
    if (sortKey === key) {
      if (sortOrder === 'desc') {
        setSortOrder('asc');
      } else {
  ...
  const filteredAndSortedVideos = useMemo(() => {
    // 1. Filter by active tab (always posts for now since drafts is mock)
    ...
    return result;
  }, [videos, activeTab, searchQuery, privacyFilter, pinnedVideoIds, sortKey, sortOrder]);
          ...
          <table className="w-full text-left border-collapse min-w-[850px]">
            ...
            <tbody className="divide-y divide-elevated">
              {filteredAndSortedVideos.length === 0 ? (
                ...
                filteredAndSortedVideos.map((video: Video) => {
