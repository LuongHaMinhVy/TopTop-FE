        ...
        {attachment && (
          ...
          >
            {attachment.thumbnailUrl ? (
              <img src={attachment.thumbnailUrl} alt="Video cover" className="w-full h-full object-cover opacity-90 group-hover:opacity-100 transition" />
            ) : (
            ...
            <div className="absolute bottom-0 inset-x-0 bg-gradient-to-t from-black/80 via-black/40 to-transparent p-3 pointer-events-none flex flex-col justify-end">
              <div className="flex items-center gap-2">
                <div className="size-5 rounded-full bg-white/20 overflow-hidden flex-shrink-0">
                  {attachment.thumbnailUrl && (
                    <img src={attachment.thumbnailUrl} alt="" className="w-full h-full object-cover" />
                  )}
                </div>
    ...
    return (
      <div className={`flex flex-col ${message.mine ? 'items-end' : 'items-start'}`}>
        ...
        <div className="mt-1 max-w-[240px] overflow-hidden rounded-xl bg-black">
          {message.type === 'IMAGE' ? (
            <img src={mediaUrl} alt={attachment.fileName || "Image"} className="max-h-[320px] w-full object-cover" />
          ) : (
