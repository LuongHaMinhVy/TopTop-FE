...
import {
  DocumentTitle,
  truncateTitle,
} from "@/components/shared/DocumentTitle";
import {
  ChevronLeft,
  Heart,
  Link as LinkIcon,
...
const DIRECT_DETAIL_SOURCES = new Set([
  "feed",
  "friends",
  "following",
  "follow",
]);
const PROFILE_DETAIL_SOURCES = new Set([
  "profile",
  "collection",
  "favorites",
  "liked",
...
function rotateVideosFromSelected(videos: Video[], selectedVideo: Video) {
  const selectedIndex = videos.findIndex((item) => item.id === selectedVideo.id);

  if (selectedIndex < 0) {
    return [selectedVideo, ...videos.filter((item) => item.id !== selectedVideo.id)];
  }

  return [
    videos[selectedIndex],
    ...videos.slice(selectedIndex + 1),
    ...videos.slice(0, selectedIndex),
  ];
}
...
export default function VideoDetailPage() {
  ...
  const detailMode: DetailMode =
    !source || DIRECT_DETAIL_SOURCES.has(source) ? "direct" : "internal";
  const isProfileDetail = source === "profile";
  const isCollectionDetail =
    source === "collection" &&
    Number.isFinite(collectionId) &&
    Number(collectionId) > 0;
  const usesProfileDetailLayout =
    isProfileDetail || PROFILE_DETAIL_SOURCES.has(source ?? "");
  ...
  const defaultTab: DetailTab = commentsRequested
    ? "comments"
    : detailMode === "direct"
      ? "videos"
      : "comments";
  ...
  const {
    ...
    isLoading,
    isError,
  } = useVideoDetail(username, videoId);
  ...
  const activeTab =
    activeTabState.key === activeTabKey ? activeTabState.tab : defaultTab;
  ...
  const [activeScrollableVideo, setActiveScrollableVideo] =
  ...
  const blockMutation = useBlockUserMutation(
    displayedVideo?.username ?? username,
  );
  ...
  const { data: repostedVideosRes } = useUserRepostedVideos(
    profileOwner,
    source === "reposts",
  );
  ...
  const { data: friendsFeedRes } = useFriendsFeed(
    Boolean(currentUser) && source === "friends",
    30,
  );
  const { data: searchVideosRes } = useSearchVideos(
    searchQuery,
    0,
    50,
    source === "search" && searchQuery.length > 0,
  );
  ...
  const detailScopeParams = useMemo(() => {
    if (source === "search")
      return { from: "search", q: searchQuery || undefined };
  ...
  const scrollableVideos = useMemo(() => {
    ...
    if (source === "profile") {
      ...
      videos = (collectionVideosRes?.data ?? []).filter(
        (item) => !item.unavailable && !item.deleted,
      );
    } else if (source === "favorites") {
      videos = (favoriteVideosRes?.data ?? []).filter(
        (item) => !item.unavailable && !item.deleted,
      );
    } else if (source === "liked") {
      videos = (likedVideosRes?.data ?? []).filter(
        (item) => !item.unavailable && !item.deleted,
      );
    } else if (source === "reposts") {
      videos = (repostedVideosRes?.data ?? []).filter(
        (item) => !item.unavailable && !item.deleted,
      );
    } else if (source === "chat") {
      ...
      if (convIdStr) {
        ...
        if (chatVideoIdsStr) {
          ...
          videos = ids
            .map((id) => allVideosRes?.data?.find((v) => v.id === id))
    ...
    if (
      ...
      return rotateVideosFromSelected(videos, video);
    }
  ...
  const repostUsers = (() => {
    ...
    const hasCurrentUser =
      currentRepostUser &&
      users.some((user) => user.id === currentRepostUser.id);
  ...
  const canonicalPath = displayedVideo
    ? videoPath(displayedVideo.username, displayedVideo.id)
    : "";
  const shareUrl =
  ...
  const handleSave = () => {
    ...
    setOptimisticVideo({
      ...
      saveCount: Math.max(
        0,
        (displayedVideo.saveCount ?? 0) + (nextSaved ? 1 : -1),
      ),
    });
  ...
  const handleToggleFollowAuthor = () => {
    ...
    mutation.mutate(displayedVideo.username, {
      onError: () =>
        setFollowOverride({
          userId: displayedVideo.userId,
          value: previousFollowing,
        }),
    });
  };
  ...
  const navigateToVideo = useCallback(
    (direction: 1 | -1) => {
      ...
      const currentIndex = scrollableVideos.findIndex(
  ...
  useEffect(() => {
    if (
      (isUnavailable || isError || (!isLoading && !videoRes?.data)) &&
      source === "chat"
    ) {
    ...
    return (
      <div className="relative flex h-full w-full overflow-hidden bg-background">
          ...
          >
            {scrollableVideos.map((scrollVideo) => (
              <ScrollableFeedDetailVideo
                ...
                stageOffsetX={
                  usesFeedThemeDetail ? "clamp(-80px, -4vw, -48px)" : "0px"
                }
              />
            ))}
          </div>
        ...
        <aside
          ...
          style={{
            width: usesFeedThemeDetail
              ? FEED_DETAIL_PANEL_WIDTH
              : DIRECT_DETAIL_PANEL_WIDTH,
          }}
        >
          <div className="h-24" />
    ...
    <div className="fixed inset-0 z-[100] flex overflow-hidden bg-[#111] text-white animate-in fade-in duration-200">
      ...
      <aside
        className={`hidden flex-shrink-0 flex-col border-l border-white/10 bg-background xl:flex ${
          usesProfileDetailLayout
            ? "w-[34vw] min-w-[420px] max-w-[560px]"
            : "w-[520px]"
        }`}
      >
        <div className="border-b border-white/10 p-5">
          <div className="mb-4 rounded-xl bg-elevated/70 p-4">
            ...
            <SoundBadge
              sound={displayedVideo.sound}
              fallbackLabel={t("originalSound", { username: displayedVideo.username })}
              className="mt-2.5 text-text-secondary hover:text-text-primary"
            />
            <RepostBadge
              users={repostUsers}
              onRepost={canRepost && !isReposted ? handleRepost : undefined}
          ...
          <div className="mb-4 flex items-center gap-3">
            <CompactMetric
              ...
              icon={
                <Heart
                  className={displayedVideo.isLiked ? "fill-current" : ""}
                  size={20}
                />
              }
            ...
            <CompactMetric
              ...
              icon={
                <Bookmark
                  className={displayedVideo.isSaved ? "fill-current" : ""}
                  size={20}
                />
              }
    ...
    <div className="grid grid-cols-2 gap-4 p-6">
      {videos.map((video) => (
        ...
        >
          ...
          <p className="mt-2 line-clamp-2 text-[15px] font-bold leading-snug">
            {video.title}
          </p>
    ...
    <div
      className={
        mode === "direct"
          ...
          : "fixed inset-0 z-[100] flex bg-background"
      }
    >
      ...
      <div className="hidden w-[420px] flex-col gap-6 p-6 lg:flex">
        ...
        <div className="grid grid-cols-2 gap-4">
          {[1, 2, 3, 4].map((item) => (
            <div
              key={item}
              className="aspect-[3/4] rounded-lg bg-neutral-800"
            />
          ))}
        </div>
      </div>
