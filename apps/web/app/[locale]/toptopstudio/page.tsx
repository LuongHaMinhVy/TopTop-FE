...
export default function StudioDashboard() {
  ...
  const commentQueries = useQueries({
    queries: videos.map((video) => ({
      queryKey: ['studio-comments', video.id],
  ...
  const { data: dailyViewsResponse } = useStudioDailyViews(selectedDays, !!user);
  ...
  const totalShares = useMemo(() => videos.reduce((sum, v) => sum + (v.shareCount || 0), 0), [videos]);
  ...
  const metricDates = useMemo(() => {
    ...
    const dailyViewsByDate = new Map(
      (dailyViewsResponse?.data ?? []).map((item) => [item.date, item.views ?? 0]),
    );

    const today = new Date();
    return Array.from({ length: selectedDays }, (_, index) => {
      const date = new Date(today);
      const daysAgo = selectedDays - 1 - index;
      ...
      return {
        ...
        views: dailyViewsByDate.get(dateKey) ?? 0,
      };
    });
  }, [dailyViewsResponse?.data, locale, selectedDays]);

  const buildMetricPoints = useCallback(
    (selector: (video: (typeof videos)[number]) => number) =>
      metricDates.map(({ start, end }) =>
        videos.reduce((sum, video) => {
          const createdAt = new Date(video.createdAt).getTime();
          if (createdAt >= start.getTime() && createdAt < end.getTime()) {
            return sum + selector(video);
          }
          return sum;
        }, 0),
      ),
  ...
  const formatTrend = (value: number) => (value === 0 ? '0 (-)' : `+${value.toLocaleString()} (100%)`);
    ...
    return commentQueries
      .flatMap((query, index) => {
        ...
        return (query.data?.data ?? []).map((comment) => ({
          id: comment.id,
          author: comment.author?.displayName || comment.username,
          avatarUrl: comment.author?.avatarUrl || comment.userAvatarUrl,
      ...
      .slice(0, 5);
  }, [commentQueries, videos, t]);

  const chartData = useMemo(() => {
    const viewsPoints = metricDates.map((date) => date.views);
    const likesPoints = buildMetricPoints((video) => video.likeCount || 0);
    const commentsPoints = buildMetricPoints((video) => video.commentCount || 0);
    const sharesPoints = buildMetricPoints((video) => video.shareCount || 0);
    const localizedDates = metricDates.map((date) => date.label);

    return {
      views: {
        title: t('home.videoViewsTab'),
        value: totalViews.toLocaleString(),
        trend: formatTrend(viewsPoints.reduce((sum, value) => sum + value, 0)),
        points: viewsPoints,
      ...
      profile_views: {
        ...
        value: (user?.videoCount ?? videos.length).toLocaleString(),
        trend: formatTrend(videos.length),
        points: buildMetricPoints(() => 1),
        dates: localizedDates,
        color: '#8b5cf6'
      },
      likes: {
        ...
        trend: formatTrend(likesPoints.reduce((sum, value) => sum + value, 0)),
        points: likesPoints,
        dates: localizedDates,
        color: '#ef4444'
      },
      comments: {
        ...
        trend: formatTrend(commentsPoints.reduce((sum, value) => sum + value, 0)),
      ...
      shares: {
        ...
        trend: formatTrend(sharesPoints.reduce((sum, value) => sum + value, 0)),
        points: sharesPoints,
        dates: localizedDates,
        color: '#f59e0b'
      },
      earnings: {
        ...
        value: '$0,00',
        trend: '0,00 (0.0%)',
        points: localizedDates.map(() => 0),
        dates: localizedDates,
        color: '#06b6d4'
      }
    };
  }, [buildMetricPoints, metricDates, t, totalViews, totalLikes, totalComments, totalShares, user?.videoCount, videos]);
  ...
  const paddingX = 50;
  const paddingY = 20;
  const chartTop = paddingY + 6;
  const chartBottom = svgHeight - paddingY - 8;

  const points = activeData.points;
  const maxVal = Math.max(...points);
  const hasChartData = maxVal > 0;

  const coordinates = useMemo(() => {
    const totalPoints = points.length;
    const stepX = totalPoints > 1 ? (svgWidth - paddingX * 2) / (totalPoints - 1) : 0;
    
    return points.map((val, idx) => {
      const x = paddingX + idx * stepX;
      const y = hasChartData
        ? chartBottom - ((val / maxVal) * (chartBottom - chartTop))
        : chartBottom;
      return { x, y, value: val };
    });
  }, [chartBottom, chartTop, hasChartData, maxVal, points]);
  ...
  const areaD = useMemo(() => {
    if (coordinates.length === 0) return '';
    return `${pathD} L ${coordinates[coordinates.length - 1].x} ${chartBottom} L ${coordinates[0].x} ${chartBottom} Z`;
  }, [chartBottom, coordinates, pathD]);
  ...
  const displayName = user?.nickname?.trim() || user?.username?.trim() || t('title');
  const usernameLabel = user?.username ? `@${user.username}` : "";
  const avatarInitial = (displayName || "T").trim()[0]?.toUpperCase() ?? "T";
  const isAvatarBroken = Boolean(user?.avatarUrl && brokenAvatarUrl === user.avatarUrl);
  const profileHref = user?.username ? `/@${user.username}` : "/toptopstudio";
        ...
        <div className="flex items-center gap-5">
          ...
          <div className="relative w-16 h-16 rounded-full overflow-hidden border border-elevated shrink-0 bg-surface flex items-center justify-center font-bold text-xl text-white/80">
            {user?.avatarUrl && !isAvatarBroken ? (
              <Image
                ...
                unoptimized
                onError={() => setBrokenAvatarUrl(user.avatarUrl ?? null)}
              />
            ) : (
              avatarInitial
            )}
          ...
          <div className="flex flex-col gap-1">
            <div className="flex items-center gap-2">
              ...
              {usernameLabel && <span className="text-xs text-text-muted">{usernameLabel}</span>}
            </div>
            {user?.bio?.trim() && (
            ...
            <div className="flex items-center gap-3 mt-1.5 text-xs text-text-muted">
              ...
              <span>{t('home.followersCount')} <strong className="text-text-primary font-bold">{user?.followersCount ?? 0}</strong></span>
              <span className="text-white/10">•</span>
              <span>{t('home.followingCount')} <strong className="text-text-primary font-bold">{user?.followingCount ?? 0}</strong></span>
            </div>
          </div>
        </div>
                ...
                >
                  ...
                  <div className="flex items-center gap-5 text-text-secondary shrink-0 select-none">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <span className="text-xs font-bold">{video.shareCount || 0}</span>
                    </div>
                  </div>
                </div>
              ))
          ...
          <div className="flex flex-col gap-4 flex-1">
            {latestComments.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-12 text-center text-text-secondary">
                ...
                <p className="text-sm font-semibold">{t('home.noComments')}</p>
              </div>
